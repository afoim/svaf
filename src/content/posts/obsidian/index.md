...
首先，确保你将 `src/content` 作为仓库根目录，因为 `src/content/posts` 存放博客文章，而 `src/content/spec` 存放关于等特殊MarkDown页面，他们都可能需要依赖图片，所以建议将仓库设置在他们的上一级文件夹。我们的图片将存放在 `src/content/public/assets/images` 在 `posts` 或 `spec` 的相对路径引用格式则为 `img/xxx.webp` （不用担心，黑曜石会自动管理，你无需手打）
点击 **设置** 
![](/public/assets/images/obsidian-3.webp)
如图配置，这样我们就解决了第一个问题
![](img/obsidian-4.webp)
