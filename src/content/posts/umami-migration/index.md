...
然后连上数据库，你将可以看到这些表
![](img/umami-migration-4.webp)

接下来我们查看 `website` 这张表，将 `website_event.csv` 打开，查看旧的 `website_id` 将新的ID改为旧的，这样才能数据匹配
![](img/umami-migration-5.webp)

![](img/umami-migration-6.webp)
...
![](img/umami-migration-7.webp)

编辑CSV文件，你需要将顺序配对，并且删除表中没有的列，如： `browser` `os` ...
![](img/umami-migration-8.webp)

确保数据库中的表头顺序和CSV文件中的表头顺序一一对应，顺序一致，不多不少后，**Ctrl + S 保存** CSV文件，我们开始导入并在选项中勾选 **标题**

![](img/umami-migration-9.webp)

![](img/umami-migration-10.webp)

![](img/umami-migration-11.webp)
导入成功后如图
![](img/umami-migration-12.webp)

![](img/umami-migration-13.webp)

再编辑 `session` 这张表，手法同上，导入成功后如图
![](img/umami-migration-14.webp)

接下来打开本地Umami的WebUI，查看数据是否有异常
![](img/umami-migration-15.webp)
